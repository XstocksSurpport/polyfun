...
if (existsSync(deploymentPath)) {
  ...
  if (d.genesisMarket) {
    console.log("Genesis market exists:", d.genesisMarket);
    process.exit(0);
  }
}

const prep = spawnSync("node", ["prep-genesis.mjs"], {
...
const script = spawnSync(
  ...
  [
    "script",
    "script/BootstrapGenesis.s.sol:BootstrapGenesis",
    "--rpc-url",
    rpc,
    "--broadcast",
    "--slow",
  ...
  ],
  {
    ...
    env: {
      ...
      GENESIS_SALT: salt,
      GENESIS_METADATA_HASH: metadataHash,
    },
  }
);
...
const boot = JSON.parse(readFileSync(join(root, "deployments", "genesis-bootstrap.json"), "utf8"));
...
writeFileSync(
  ...
  Object.entries({ ...envLocal, NEXT_PUBLIC_GENESIS_MARKET_ADDRESS: boot.genesisMarket })
    .map(([k, v]) => `${k}=${v}`)
    .join("\n") + "\n"
);

console.log("Genesis market:", boot.genesisMarket);
console.log("Trade:", `http://localhost:3001/market/${boot.genesisMarket}`);
